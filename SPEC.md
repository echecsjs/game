...
These pieces are as follows: A white king (K), a white queen (Q), two white rooks (R), two white bishops (B), two white knights (N), eight white pawns; and the same for Black.

2.3 The initial position of the pieces on the chessboard is as follows: The first rank for White (from left to right): Ra1, Nb1, Bc1, Qd1, Ke1, Bf1, Ng1, Rh1; second rank: pawns on a2–h2. Symmetrically for Black on ranks 7 and 8.
...
6.1 'Chessclock' means a clock with two time displays, connected to each other in such a way that only one of them can run at a time. 'Clock' in the Laws of Chess means one of the two time displays. Each time display has a 'flag'. 'Flag-fall' means the expiration of the allotted time for a player.
...
11.2.3 Only with the permission of the arbiter can: a player leave the playing venue, the player having the move be allowed to leave the playing area, or a person who is neither a player nor arbiter be allowed access to the playing area.
...
A.5 Otherwise the following apply: [abbreviated – see A.5.1 through A.5.5 in the full text]
...
B.2 The Competitive Rules of Play shall apply if: one arbiter supervises one game and each game is recorded by the arbiter or his/her assistant and, if possible, by electronic means.
...
FIDE recognises for its own tournaments and matches only one system of notation, the Algebraic System.
...
C.3 For the abbreviation of the name of the pieces, each player is free to use the name which is commonly used in his/her country.
...
C.8 Each move of a piece is indicated by the abbreviation of the name of the piece in question and the square of arrival. There is no need for a hyphen between name and square. Examples: Be5, Nf3, Rd1. In the case of pawns, only the square of arrival is indicated. Examples: e5, d4, a5.

C.9 When a piece makes a capture, an x may be inserted between the abbreviation of the name of the piece and the square of arrival. Examples: Bxe5, Nxf3, Rxd1.
...
C.10 If two identical pieces can move to the same square, the piece that is moved is indicated as follows: if both pieces are on the same rank by the file of departure; if both pieces are on the same file by the rank of departure; if the pieces are on different ranks and files, method 1 is preferred.
...
[See full text at https://handbook.fide.com/chapter/E012023]

## Guidelines I. Adjourned Games

I.1 Adjournment procedure: If a game is not finished at the end of the time prescribed for play, the arbiter shall require the player having the move to 'seal' that move. The player must write his/her move in unambiguous notation on a paper scoresheet, put his/her scoresheet and that of his/her opponent in an envelope, seal the envelope and only then stop the chessclock.

[Articles I.2 through I.14 govern the handling and resumption of adjourned games.]

## Guidelines II. Chess960 Rules

II.1 Before a Chess960 game a starting position is randomly set up, subject to certain rules. After this, the game is played in the same way as regular chess.

II.2 Starting Position Requirements: White pawns are placed on the second rank as in regular chess. All remaining white pieces are placed randomly on the first rank, but with the following restrictions: the king is placed somewhere between the two rooks; the bishops are placed on opposite-coloured squares; the black pieces are placed opposite the white pieces.

II.3 Chess960 castling rules: Chess960 allows each player to castle once per game. After castling, the rook and king's final positions should be exactly the same positions as they would be in regular chess.

II.3.2.6 After c-side castling (notated as 0-0-0), the king is on the c-square (c1 for white and c8 for black) and the rook is on the d-square. After g-side castling (notated as 0-0), the king is on the g-square and the rook is on the f-square.
...
III.4 If the player having the move has less than two minutes left on his/her clock, he/she may request that an increment extra five seconds be introduced for both players. This constitutes the offer of a draw.

III.5 If Article III.4 does not apply and the player having the move has less than two minutes left on his/her clock, he/she may claim a draw before his/her flag falls.
...
| capture | 3.1 | Where a piece is moved from its square to a square occupied by an opponent's piece, the latter is removed from the board. In notation x. |
| castling | 3.8.2 | A move of the king towards a rook. In notation 0-0 kingside castling, 0-0-0 queenside castling. |
...
| en passant | 3.7.4.1 | A pawn capture immediately following an opponent's double-pawn advance. In notation e.p. |
